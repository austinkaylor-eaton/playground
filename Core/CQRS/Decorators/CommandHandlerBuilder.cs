    ...
    where THandler : class, ICommandHandler<TCommand, TResponse>
{
    ...
    /// <summary>
    /// Adds the metrics decorator to the command handler chain.
    /// Records execution counts, durations, and failure rates via <see cref="CqrsMetrics"/> instruments.
    /// </summary>
    /// <returns>The builder instance for method chaining.</returns>
    public CommandHandlerBuilder<TCommand, TResponse, THandler> WithMetrics()
    {
        _decoratorFactories.Add((_, inner) =>
            new MetricsCommandHandler<TCommand, TResponse>(inner));
